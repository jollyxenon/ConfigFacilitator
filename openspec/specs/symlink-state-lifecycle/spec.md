# symlink-state-lifecycle Specification

## Purpose
Define the lifecycle for ConfigFacilitator managed target state, including hard-link-backed regular-file activation, ownership checks, rollback, reset, update, revert, and force recovery.
## Requirements
### Requirement: Apply uses hard links for file targets only
...
### Requirement: Directory targets are unsupported by the hard-link backend
...
### Requirement: Column update preserves non-selected current mappings
When update is scoped to a column, the system SHALL recompute mappings for only the selected column and combine them with the unchanged current mappings for all other columns before committing the new project state. If current state records a mode apply intent, the selected column SHALL be re-planned using the mode's strategy for that column.

#### Scenario: Preserving other columns during column update
- **WHEN** a project has active mappings for multiple columns and update runs with `-c <ColumnName>`
- **THEN** mappings for the selected column are refreshed from current warehouse metadata
- **AND** mappings for non-selected columns remain in the committed current state as they were before the update

#### Scenario: Column update with full mode intent includes new settings
- **WHEN** a project has current state from a mode apply and the selected column uses `strategy: full`
- **AND** the selected column has a new setting in current warehouse metadata
- **THEN** column-scoped update includes that new setting in the selected column's refreshed mappings

#### Scenario: Column update with no active mappings for that column
- **WHEN** update runs with `-c <ColumnName>` and the selected column has no active current managed mappings and no persisted intent for that column
- **THEN** the CLI reports that there is nothing to update for that column and leaves the current state unchanged

### Requirement: Update failures preserve pre-update state
The system SHALL preserve the confirmed pre-update mapping set if an update operation fails after mutating managed links but before completing state persistence.

#### Scenario: State write fails during update
- **WHEN** update has already changed filesystem links and a later state-persistence step fails
- **THEN** the engine rolls back managed targets to the last confirmed pre-update mapping set

### Requirement: Engine failures restore the pre-operation state
The system SHALL restore the pre-operation mapping set if an engine operation fails after mutating managed links but before completing state persistence. When force was enabled, rollback SHALL restore the pre-operation managed mapping set only and SHALL NOT promise recovery of unmanaged contents that were deleted or overwritten while reclaiming target paths.

#### Scenario: State write fails after link mutation
- **WHEN** the engine has already changed filesystem links and a later state-persistence step fails
- **THEN** the engine rolls back the managed targets to the last confirmed pre-operation mapping set

#### Scenario: Forced rollback after reclaiming unmanaged content
- **WHEN** a forced operation deletes or overwrites unmanaged target content and a later persistence step fails
- **THEN** the engine restores the previous managed mapping set
- **AND** does not restore the deleted unmanaged content from backup storage
