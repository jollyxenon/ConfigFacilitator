# warehouse-structure Specification
...
### Requirement: Backup state file placement
The system SHALL keep `current_state.json` and `history.log` inside each project's `Backup/` directory and SHALL treat those files as project-scoped operational state rather than user-authored configuration templates.

#### Scenario: Looking up project backup files
- **WHEN** an apply, reset, or revert operation needs project state
- **THEN** the CLI reads and writes only the backup files located under that project's `Backup/` directory
