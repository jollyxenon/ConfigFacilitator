...
- [Agent 使用 Skill](../.opencode/skills/configfacilitator-usage/SKILL.md)
...
- Agent Skill 维护：当面向用户的命令、工作流、示例或安全规则变化时，需要随文档一起检查并更新 [`configfacilitator-usage`](../.opencode/skills/configfacilitator-usage/SKILL.md)

## 项目作用

它负责搭建仓库骨架、同步索引与磁盘实体、保存基于 PPID 的便利上下文、通过 `cfgfc root` 持久化切换仓库根目录、应用基于硬链接的常规文件配置，并支持 `reset` 和单步 `revert`。
