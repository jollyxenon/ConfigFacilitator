...
11. Web UI smoke: start `cfgfc web`, open `http://127.0.0.1:38031`, create a Column, UTF-8 file Setting, empty directory Setting, and empty Mode, then edit Column/Setting Index metadata and targets and perform a canonical rename; confirm returned snapshots update navigation without `sync`, stale revisions return `409`, a used port fails with a persistence error, and Ctrl-C exits cleanly.
