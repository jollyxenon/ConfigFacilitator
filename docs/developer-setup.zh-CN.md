...
11. Web UI 冒烟：启动 `cfgfc web`，打开 `http://127.0.0.1:38031`，创建 Column、UTF-8 文件 Setting、空目录 Setting 和空 Mode，再编辑 Column/Setting Index 元数据与目标并执行 canonical 重命名；确认返回 snapshot 后导航无需 `sync` 即更新、过期 revision 返回 `409`、端口被占用时报持久化错误、Ctrl-C 干净退出。
