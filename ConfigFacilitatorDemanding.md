...
这个软件应该是 portable（便携式）的，所有需要管理的配置默认放在统一的 `~/.configfacilitator/` 根目录中，并允许用户通过单独命令持久化切换到别的仓库根目录。系统的底层核心机制采用**软链接 (Symlink)** 驱动，并在每次应用配置时记录状态，以便于撤销和回溯。
...
1. **软链接驱动 (Symlink-based) 与防冲突**：所有的 `apply` 操作本质上是在**目标绝对路径**建立指向当前生效仓库根目录内项目真实文件的软链接。如果程序发现在准备创建软链接的 `target` 路径上，**已经存在非本系统生成的真实物理文件/文件夹**，程序必须挂起并**提示用户**（确认是否覆盖、备份或跳过），绝不能悄无声息地覆盖用户原有数据。
