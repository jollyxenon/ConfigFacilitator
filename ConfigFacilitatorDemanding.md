...
这个软件应该是 portable（便携式）的，所有需要管理的配置直接放在统一的 `~/.configfacilitator/` 根目录中。系统的底层核心机制采用**软链接 (Symlink)** 驱动，并在每次应用配置时记录状态，以便于撤销和回溯。
...
系统直接把 `~/.configfacilitator/` 视为 canonical 仓库根，并从这个根目录下发现项目目录。

`~/.configfacilitator/` 根目录下除了各个项目文件夹外，还包含一个 `ProjectIndex.jsonc` 文件，记录每个项目的默认显示名和别名。
