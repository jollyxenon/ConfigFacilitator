...
这个软件应该是 portable（便携式）的，所有需要管理的配置默认放在统一的 `~/.configfacilitator/` 根目录中，并允许用户通过单独命令持久化切换到别的仓库根目录。系统的底层核心机制采用**硬链接 (Hard Link)** 驱动，并在每次应用配置时记录状态，以便于撤销和回溯。

## 简单示例

- 在 `~/.configfacilitator/` 根目录中可以有不同的项目，如 `Claude Code`、`OpenCode` 等。
...
所有的索引文件均采用 `.jsonc` 格式。系统在生成空模板时，会在文件末尾追加一个长段的示例注释块来提供书写指引（告诉用户哪些项可以填什么）。**当用户完成书写并在随后执行同步时，这个示例注释块被程序抹除是正常且被允许的行为**。用户必须统一使用 JSON 中的 `"description"` 字段来承载永久性的说明与备注，该字段在数据序列化时绝对不会丢失。

系统默认把 `~/.configfacilitator/` 视为 canonical 仓库根，并允许通过持久化 root 配置切换到新的生效仓库根目录。项目目录总是从当前生效根目录下发现。

当前生效仓库根目录下除了各个项目文件夹外，还包含一个 `ProjectIndex.jsonc` 文件，记录每个项目的默认显示名和别名。

**项目层级划分**：每个项目文件夹下分为 `Column`、`Mode` 和 `Backup` 三个专门的子目录。

- **Backup 文件夹规则**：存放状态与应用历史。包含 `current_state.json`（记录当前项目在系统中建立了哪些受管路径映射）以及 `history.log`（历史操作的时间戳记录）。
- **Column 文件夹规则**：
...
1. **硬链接驱动 (Hard-link-based) 与防冲突**：所有的 `apply` 操作本质上都是在**目标绝对路径**建立指向当前生效仓库根目录内项目真实文件内容的文件硬链接。它只支持常规文件；目录型映射必须清晰失败，且绝不退回到 symlink、junction、复制、`mklink`、PowerShell 或其他替代机制。硬链接通常不能跨文件系统或跨 Windows 卷。如果程序发现在准备创建目标的 `target` 路径上，**已经存在非本系统生成的真实物理文件/文件夹**，程序必须挂起并**提示用户**（确认是否覆盖、备份或跳过），绝不能悄无声息地覆盖用户原有数据。
2. **状态记录与回溯**：在每次 `apply` 前，工具会读取当前项目的 `Backup/current_state.json`，清理需要撤销的旧受管路径，然后再生成新的硬链接目标。新生成的映射表会被重新写入该状态文件。由于硬链接两端共享同一份底层内容，所以无论编辑仓库 source，还是编辑激活后的 target，改动都会互相可见。
...
**注意**：`switch` 会为当前终端的 PPID 作用域保存活动项目上下文。之后，项目作用域内的 `new`、`sync`、`list`、`apply`、`reset` 和 `revert` 都可以省略 `-p <ProjectName>`；如果同时显式传入 `-p`，则显式项目优先。

### 1. 搭建骨架 (`new`)

*说明：每次使用 `new` 指令时，自动生成带有末尾示例注释块与 `"description"` 字段的空模板。*

- **新建项目**：`cfgfc new -p <ProjectName>`。生成标准化的 `Column`、`Mode` 与 `Backup` 目录，并更新 `ProjectIndex.jsonc`。
- **新建栏目**：`cfgfc new -p <ProjectName> -c <ColumnName>`。在指定的项目下新建一个栏目文件夹，并生成基础的 `SettingIndex.jsonc`。如果已经执行过 `cfgfc switch <ProjectName>`，也可以直接使用 `cfgfc new -c <ColumnName>`。
- **新建模式**：`cfgfc new -p <ProjectName> -m <ModeName>`。在 `ModeIndex.jsonc` 中新增一个模板，自动列出当前已有的栏目供用户填空。如果已经执行过 `cfgfc switch <ProjectName>`，也可以直接使用 `cfgfc new -m <ModeName>`。
- **保留字说明**：`global` 为保留字，不能作为项目名使用。

### 2. 同步数据 (`sync`)

- **上下文同步 / 全局同步**：`cfgfc sync`。如果当前存在通过 `switch` 选中的活动项目，则只扫描并同步该项目；如果当前没有活动项目，则自动扫描 `~/.configfacilitator/` 下所有项目及栏目的实体文件，补全和修复所有的 Index 记录。
- **项目同步**：`cfgfc sync -p <ProjectName>`。仅扫描并同步指定项目。
- **强制全仓同步**：`cfgfc sync --all` 或 `cfgfc sync -a`。无论当前是否存在 `switch` 上下文，都会强制刷新整个仓库的索引。
- **保留字说明**：`cfgfc sync -p global` 会直接报错，因为 `global` 为保留字。
...
- **返回全局 (`switch global`)**：清除当前终端的活动项目上下文，后续命令回到原有的全局解析模式。
...
- **底层行为**：防冲突检测 -> 清理需撤销的旧受管路径 -> 解析路径变量并创建新的文件硬链接 -> 写入新状态与时间戳日志。

### 5. 重置与回退 (`reset` / `revert`)

- **重置环境 (`reset`)**：`cfgfc reset -p <ProjectName>`
  - **底层行为**：读取 `Backup/current_state.json` 中的受管路径列表，将它们从系统中安全移除（撤销该项目当前产生的所有目标路径），并清空状态文件，将目标环境彻底还原为无配置的干净状态。

- **回退历史 (`revert`)**：`cfgfc revert -p <ProjectName>`
  - **底层行为**：读取 `Backup/history.log` 与历史状态记录，解析出上一次执行 `apply` 操作前的受管映射状态。系统会先安全清理当前目标路径，随后按照历史记录重新建立之前的文件硬链接，将环境精准回退到上一个配置版本，而非单纯清空。
...
2. **填充内容与新建模式模板**：
    ...
    - 用户打开对应的 `SettingIndex.jsonc` 填写 `"defaultTargetDir"` / `"defaultTargetName"` 或 Setting 内的 `"targetDir"` / `"targetName"`，指定硬链接激活的目标目录和名称（目录可使用 `~` 等变量；但真正可激活的 source 必须是常规文件，且通常要和 target 位于同一文件系统或同一 Windows 卷）。
    - 生成模式模板：
    ...
    - 用户使用文本编辑器打开生成的 `~/.configfacilitator/OpenCode/Mode/ModeIndex.jsonc`，补全映射关系和覆盖策略。
...
4. **应用配置**：
    ...
    *(控制台输出：已成功清理旧目标/保留增量，并将相关配置以文件硬链接形式激活到目标路径...)*
